...
class MiniMaxOpening:
    ...
    def estimate_opening(self, board):
        ...
        return 1000 * (white - black) + 1000 * (white_mills - black_mills)
    ...
    def generate_moves_opening(self, board, is_white):
        ...
        for i in range(21):
            if temp_board[i] == 'x':
                new_board = temp_board[:i] + 'W' + temp_board[i+1:]
                if self.closeMill(i, new_board):
                    removed = False
                    for j in range(21):
                        if new_board[j] == 'B' and not self.closeMill(j, new_board):
                            modified = new_board[:j] + 'x' + new_board[j+1:]
                            result.append(modified if is_white else self.flip_colors(modified))
                            removed = True
                    if not removed:
                        result.append(new_board if is_white else self.flip_colors(new_board))
                else:
                    result.append(new_board if is_white else self.flip_colors(new_board))
        return result
...
if __name__ == "__main__":
    ...
    if len(start_board) != 21:
        start_board = start_board.ljust(21)[:21]
